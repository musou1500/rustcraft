...
impl Terrain {
    ...
    fn generate_chunk_data(
        ...
        ChunkData,
        [[[BlockType; WORLD_HEIGHT]; CHUNK_SIZE]; CHUNK_SIZE],
    ) {
        ...
        let noise_data: Vec<(usize, usize, usize, f64)> = (0..CHUNK_SIZE)
            ...
            .flat_map(|x| {
                (0..CHUNK_SIZE).into_par_iter().map(move |z| {
                    ...
                    let combined_noise =
                        height_noise1 * 0.6 + height_noise2 * 0.3 + height_noise3 * 0.1;
                ...
                .map(|chunk_pos| {
                    ...
                    for x in 0..CHUNK_SIZE {
                        for z in 0..CHUNK_SIZE {
                            ...
                            let height_noise1 = height_noise
                                .get([world_x as f64 * scale1, world_z as f64 * scale1]);
                            let height_noise2 = height_noise
                                .get([world_x as f64 * scale2, world_z as f64 * scale2]);
                            let height_noise3 = height_noise
                                .get([world_x as f64 * scale3, world_z as f64 * scale3]);
                            ...
                            let height = BASE_HEIGHT
                                + ((combined_noise + 1.0) * 0.5 * height_variation) as usize;

                            let biome_noise_val =
                                biome_noise.get([world_x as f64 * 0.01, world_z as f64 * 0.01]);
                    ...
                    let (chunk_data, block_array) =
                        self.generate_chunk_data(chunk_pos, &structures);
    ...
    fn generate_mesh_from_blocks(
        &self,
        chunk_pos: ChunkPos,
        chunk_blocks: &[[[BlockType; WORLD_HEIGHT]; CHUNK_SIZE]; CHUNK_SIZE],
    ) -> ChunkData {
        let mut vertices = Vec::new();
        let mut indices: Vec<u32> = Vec::new();
        let registry = get_block_registry();
