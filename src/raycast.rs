...
pub fn raycast_blocks(ray: Ray, max_distance: f32, world: &World) -> Option<RaycastHit> {
    ...
    let delta_dist = [
        if ray.direction.x.abs() < f32::EPSILON { f32::INFINITY } else { (1.0 / ray.direction.x).abs() },
        if ray.direction.y.abs() < f32::EPSILON { f32::INFINITY } else { (1.0 / ray.direction.y).abs() },
        if ray.direction.z.abs() < f32::EPSILON { f32::INFINITY } else { (1.0 / ray.direction.z).abs() },
    ];
    ...
    for i in 0..3 {
        if step[i] > 0 {
            side_dist[i] = (current_block[i] as f32 + 1.0 - match i {
                0 => ray.origin.x,
                1 => ray.origin.y,
                _ => ray.origin.z,
            }) * delta_dist[i];
            ...
            side_dist[i] = (match i {
                ...
                1 => ray.origin.y,
                _ => ray.origin.z,
            } - current_block[i] as f32) * delta_dist[i];
        }
    }
    ...
    for _ in 0..max_steps {
        ...
        if world.is_block_solid(current_block[0], current_block[1], current_block[2]) {
            ...
            let distance = match last_side {
                ...
                1 => (current_block[1] as f32 - ray.origin.y + if step[1] > 0 { 0.0 } else { 1.0 }) / ray.direction.y,
                _ => (current_block[2] as f32 - ray.origin.z + if step[2] > 0 { 0.0 } else { 1.0 }) / ray.direction.z,
            };
